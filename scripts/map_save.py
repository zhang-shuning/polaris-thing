...
def write_map(map:list[tuple[str, tuple[int]]], number = -1):
    ...
    if number < 0:
        with open(LATEST_PATH, "r", encoding="utf-8") as f:
            number = f.read()
    with open(MAP_FOLDER_PATH / f"map{number}.json") as f:
        f.write(json.dumps(map))

def read_map(number):
    ensure_save_folder_exists()
    if not (MAP_FOLDER_PATH / f"{number}.json").exists():
        return None
    else:
        with open(MAP_FOLDER_PATH / f"{number}.json", "r", encoding="utf-8") as f:
            return json.load(f)